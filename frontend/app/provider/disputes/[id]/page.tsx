...
const STATUS_META: Record<string, { label: string; bg: string; color: string }> = {
    WAITING_FOR_PROVIDER: { label: 'Cần bạn phản hồi', bg: C.yellowLight, color: C.yellow },
...
export default function ProviderDisputeDetailPage() {
    ...
    const sendMsg = async () => {
        ...
        setSending(true);
        try {
            await providerDisputeApi.sendMessage(id, msgBody.trim() || 'Đính kèm minh chứng', urls);
            setMsgBody('');
            setUploads([]);
            await load();
            setReadAt(Date.now());
        ...
        return (
            <div className="min-h-screen flex items-center justify-center px-4" style={{ background: C.bg }}>
                <div className="bg-white p-6 rounded-2xl text-center w-full max-w-sm border" style={{ borderColor: C.border }}>
                    ...
                    <button onClick={() => router.push('/provider/disputes')} className="px-4 py-2 rounded-xl text-sm font-semibold text-white w-full" style={{ background: C.orange }}>Quay lại</button>
                </div>
            </div>
        );
    }
                    ...
                    {activeTab === 'overview' && (
                        <div className="flex-1 overflow-y-auto p-4 pb-8 w-full max-w-2xl mx-auto lg:p-6">
                            ...
                            <SectionCard title="Chi tiết khiếu nại" icon={<ShieldAlert size={16} style={{ color: C.red }} />}>
                                ...
                                {dispute.expectedOutcome && <InfoRow label="Mong muốn KH" value={dispute.expectedOutcome} />}
                                ...
                                <InfoRow label="Người mở" value={dispute.openedBy?.fullName || dispute.openedBy?.email || 'N/A'} />
                            </SectionCard>
                            ...
                            {request && (
                                <SectionCard title="Thông tin dịch vụ" icon={<Receipt size={16} style={{ color: C.blue }} />}>
                                    ...
                                    <InfoRow label="Khách hàng" value={customer?.fullName || 'N/A'} />
                                    <InfoRow label="SĐT khách hàng" value={customer?.phoneNumber || 'N/A'} />
                                    <InfoRow label="Email khách hàng" value={customer?.email || 'N/A'} />
                                    <InfoRow label="Tổng thanh toán dịch vụ" value={`${(dispute.payment?.totalAmount ?? 0).toLocaleString(locale === 'vi' ? 'vi-VN' : 'en-US')}₫`} />
                                    {request.createdAt && <InfoRow label="Ngày cung cấp" value={new Date(request.createdAt).toLocaleDateString('vi-VN')} />}
                                </SectionCard>
                            )}
                                        ...
                                        <div className="flex items-end gap-2">
                                            <div className="flex-1 bg-gray-50 rounded-2xl border flex flex-col pt-1 overflow-hidden transition-colors focus-within:bg-white focus-within:border-orange-500" style={{ borderColor: C.border }}>
                                                ...
                                                <div className="flex items-center px-2 pb-2 gap-2">
                                                    ...
                                                    <span className="text-[10px] font-medium" style={{ color: '#9ca3af' }}>Ảnh + video (tối đa 2 video)</span>
                                                </div>
                                            </div>
                                            <button 
                                                onClick={sendMsg} 
