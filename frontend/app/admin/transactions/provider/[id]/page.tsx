...
export default function ProviderWalletDetailPage() {
    ...
    const fetchWallet = useCallback(async () => {
        try {
            const res = await adminApi.getProviderWallet(params.id as string);
            setWallet(res);
        } catch (error) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (activeTab === 'transactions') {
                ...
                const res = await adminApi.getWalletTransactions({ ...query, userType: 'PROVIDER' });
                setData(res.items || []);
                setTotal(res.total || 0);
            } else {
                const res = await adminApi.getTopupTransactions({ ...query, userType: 'PROVIDER' });
                setData(res.items || []);
                setTotal(res.total || 0);
            }
        } catch (error) {
